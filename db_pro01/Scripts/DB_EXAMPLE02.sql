...
SELECT INSTR('sample@example.com', '@') AS Col1 --INSTR() 두번째 매개변수의 index가 어디에있는가 알려주는 함수(검색용)
	 ...
	 , INSTR('sample@example.com', 'e', 1, 2) AS Col5 -- 네번쨰 매개변수는 그에 해당하는 값만큼의 n번째 자리에 위치한 값을 반환
	 , INSTR('sample@example.com', 'e', -1, 2) AS Col6
  FROM DUAL;

SELECT LTRIM('   sample@example.com   ', ' ') AS Col1 --첫번째 매개변수에있는 문자열에서 두번째 문자열에 해당하는 문자들을 제거한뒤에 출력함 + 좌측정렬
	 ,RTRIM('   sample@example.com   ', ' ') AS Col2 --제거는 동일하고 우측정렬
	 ,TRIM('   sample@example.com   ') AS Col3 --두번째 매개변수 없으면 양쪽의 공백을 제거한다.
	 ,TRIM(LEADING '-' FROM '-----sample@example.com-----') AS Col4 --LEADING은 좌측만 제거
	 ,TRIM(TRAILING '-' FROM '-----sample@example.com-----') AS Col5 --TRAILING은 우측만 제거
	 ,TRIM(BOTH '-' FROM '-----sample@example.com-----') AS Col6  --BOTH는 양측 다 제거
  FROM DUAL;

SELECT '!' || LPAD('A', 4) AS Col1 --여백을 줄때 사용한다 (총4칸에 마지막 'A' 투입)
	 ,'!' || LPAD('AB', 4) AS Col2
	 ,'!' || LPAD('ABC', 4) AS Col3
	 ,RPAD('A', 4) || '!' AS Col4
	 ,RPAD('AB', 4) || '!' AS Col5
	 ,RPAD('ABC', 4) || '!' AS Col6
	 ,RPAD('ABC', 4, '0') || '!' AS Col7 -- 세번째 매개변수는 덧붙이려는 문자(열), 생략 시 공백문자
	 ,RPAD('ABCDE', 4) || '!' AS Col8 -- 두번째 매개변수보다 초과되는 부분의 문자열은 삭제된다음에 출력
	 ,RPAD('ABCDE', 10) || '!' AS COL9
	 ,RPAD('ABCDE', 10, '$') || '!' AS COL9
  FROM DUAL; 

SELECT SUBSTR('sample@example.com', 1, 6) AS Col1 --첫번째 매개변수의 인덱스(포함)부터 두번째 매개변수의 숫자만큼 짜른뒤에 출력한다.
	 ,SUBSTR('sample@example.com', 8, 7) AS Col2 
	 ,SUBSTR('sample@example.com', -18, 6) AS Col3 -- 첫번째 매개변수에 '-' 온다면 뒤에서부터 숫자를 샌다음에 두번째 매개변수의 숫자만큼 짜른뒤에 출력
...
SELECT ROUND(10.4) AS Col1
	 , ROUND(10.5) AS Col2
	 , ROUND(10.45) AS Col3
	 , ROUND(10.45, 1) AS Col4 -- 매개변수에 위치한 수만큼 +해서 소수점에서 반올림한다
	 , ROUND(10.456, 2) AS Col5
	 , ROUND(18.5, -1) AS Col6 --정수 자리수 첫번째에서 반올림, 매개변수가 '-'로 들어오면 소수점자리에서 정수자리로 올라온다
  FROM DUAL;
...
SELECT SYSDATE AS Col1 --현재시간(자바에서 Date)
	 , ADD_MONTHS(SYSDATE, 2) AS Col2 
	 , ADD_MONTHS(SYSDATE, -2) AS Col3
	 , LAST_DAY(SYSDATE) AS Col4 --SYSDATE 달의 마지막날조회
...
SELECT EXTRACT (YEAR FROM SYSDATE) AS Col1 --연도 추출 --EXTRACT는 DATE자료에서 뽑아내는 역할
...
SELECT TO_NUMBER('12345') AS Col
	 , TO_NUMBER('123,456', '999,999,999') AS Col2 --두번째 매개변수는 출력포멧
--	 , TO_NUMBER('123,456', '000,000,000') AS Col3 --이렇게는 안된다! 숫자형식으로 바꾸는거니까 0이 앞에 여분으로 나와 있을 수 없음
