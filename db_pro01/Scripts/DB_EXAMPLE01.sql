...
SELECT * 
  FROM DEPARTMENTS 
 WHERE DEPARTMENT_NAME IN 'IT'; --IN은 정확히 문자열과 매치되는것만 LIKE는 매개변수로온 문자열이 포함되어있으면 불러옴

SELECT * 
  FROM DEPARTMENTS 
 WHERE DEPARTMENT_NAME LIKE '%IT%'; --위의 조회와 차이점은 %사이에 들어가면 시작하는것 뿐만 아니라 IT포함이 되어있으면 불러온다
...
SELECT SALARY, COMMISSION_PCT, SALARY * (1 + COMMISSION_PCT) AS TOTAL_SALARY --알아서 테이블이 생긴다!(임시 테이블)
  FROM EMPLOYEES 
