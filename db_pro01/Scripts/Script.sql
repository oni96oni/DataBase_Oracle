...
SELECT EMPLOYEE_ID
	...
	FROM EMPLOYEES
	WHERE EMPLOYEE_ID >= 200 --이렇게 비교연산자 사용가능
	OR FIRST_NAME = 'Steven'; --문자열 리터럴 사용할때는 ' '를 사용!
...
SELECT *
	FROM EMPLOYEES
	WHERE NOT EMPLOYEE_ID >= 200; --200이상 출력
...
SELECT *
	FROM  EMPLOYEES
	WHERE NOT EMPLOYEE_ID >= 200 --200넘지않거나 100인경우 출력
	OR EMPLOYEE_ID = 100;

SELECT *
	FROM  EMPLOYEES
	WHERE NOT (EMPLOYEE_ID >= 200 --200넘거나 100인경우 제외하고  출력
	OR EMPLOYEE_ID = 100);
