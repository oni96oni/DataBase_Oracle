...
/*
	...
	NOT NULL
		NULL데이터 허용X, 컬럼레벨에서 설정 가능
		
	UNIQUE (고유키)
		중복 데이터 허용X, UK라고도 함 , 컬럼레벨하고 테이블레벨에 설정 가능
		
	PRIMARY KEY (기본키)
		NOT NULL + UNIQUE 결합된 조건
		기본키, PK라고하며 데이터 식별값으로 사용, 컬럼레벨하고 테이블레벨에 설정 가능
		+ 두 컬럼을 묶어 하나의 제약조건으로 설정시에는 두개모두 중복되어야 데이터 제한을 피할 수 있다. 즉 제약조건에있는 모든 컬럼이 중복, NULL 값이 안들어와야 막는다.
...
CREATE TABLE sample_t (  --만들때는 대/소문자 구분안해도 괜찮아!
       u_id NUMBER       -- PRIMARY KEY(행 옆에 붙이는 대신에 아래에 컬럼으로 붙이는 방법 또한 존재)
...
-- 컬럼 레벨로만 제약 조건을 추가하는 NOT NULL, CHECK 는 modify 로 추가/삭제! NULL은 ADD,DROP이 안된다 MODIFY만 가능
ALTER TABLE ref_t MODIFY note VARCHAR2(100) NOT NULL;  -- NOT NULL 제약으로 수정하여 제약조건 추가.
ALTER TABLE ref_t MODIFY note VARCHAR2(100) NULL;      -- NULL 로 수정으로 제약조건 제거.
...
COMMENT ON COLUMN sample_t.JUMIN IS '사용자 주민등록번호';

--여기서는 대/소문자 구분을 해주어야한다.
