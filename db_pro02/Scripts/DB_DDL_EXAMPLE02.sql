...
--SELECT의 조회 결과로 테이블을 생성하는 방법으로 컬럼명과 데이터 타입, 값이 복사되고 제약 조건은 NOT NULL만 복사됨 'AS SELECT ~ '가 구문
...
COMMENT ON COLUMN EMP_COMMISSION.EMPLOYEE_ID IS '커미션받는 직원 아이디';
COMMENT ON COLUMN EMP_COMMISSION.FIRST_NAME IS '커미션받는 직원 이름';
COMMENT ON COLUMN EMP_COMMISSION.LAST_NAME IS '커미션받는 직원 성';
COMMENT ON COLUMN EMP_COMMISSION.SALARY IS '커미션받는 직원 커미션 제외 급여';
COMMENT ON COLUMN EMP_COMMISSION.COMMISSION_PCT IS '커미션받는 직원 커미션 포함 급여';
